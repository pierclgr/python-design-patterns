...
# the problem with this formatted is that we're creating a new boolean object in self.caps for each character in the
# text, so we're allocating most of the data that we won't need
...
if __name__ == "__main__":
    ...
    # BetterFormattedText uses the flyweight to memorize only the range of the text that we want to capitalize and not
    # a boolean for each position